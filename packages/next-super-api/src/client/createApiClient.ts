...
export const createApiClient = <Params extends HandlerParams, Result extends HandlerResult>(routePath: string): { fetcher: ApiFetcher<Params, Result> } => {
  const fetcher: ApiFetcher<Params, Result> = async (params) => {
    const res = await fetch(routePath, { method: 'POST', body: JSON.stringify({ data: params }) })
